...
class _VideoPlayerScreenState extends ConsumerState<VideoPlayerScreen> {
  ...
  void _onLoadSuccess() {
    final isLocal = widget.args.videoUrl != null && !widget.args.videoUrl!.startsWith('http');
    
    // Dla plików lokalnych wyłączamy ładowanie natychmiast, dla sieciowych zostawiamy lekkie buforowanie
    Future.delayed(Duration(milliseconds: isLocal ? 0 : 1200), () {
      if (mounted && _isLoading && !_isExiting) {
        _timeoutTimer?.cancel();
        _resetWatchdogTimer();
        setState(() {
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    widget.args.videoUrl != null && !widget.args.videoUrl!.startsWith('http') 
                                      ? "Otwieranie pliku..."
                                      : "Łączenie ze źródłem...",
